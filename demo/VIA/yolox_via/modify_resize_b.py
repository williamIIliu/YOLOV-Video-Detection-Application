...
work_dir = "./demo/VIA/yolox_via/"


def update_resize_output_and_remove_node(onnx_path):
    onnx_model = onnx.load(onnx_path)
    graph = onnx_model.graph
    resize_op_names = ['Resize__165', 'Resize__187', 'Resize__265'] # for centernet model
    ...
    onnx.save(onnx_model, work_dir + "yolox_s_refine.onnx")
...
def replace_resize_node_to_convtranspose(onnx_path):
    ...
    # 节点shape (resize 节点的上一个conv权重shape)
    resize_165_shape = [512, 512, 1, 1]
    resize_187_shape = [256, 256, 1, 1]
    # resize_144_shape = [64, 64, 2, 2]

    # 生成weight
    weight1 = np.zeros([512, 512, 1, 1]).astype(np.float32)
    weight2 = np.zeros([256, 256, 1, 1]).astype(np.float32)
    ...
    # define weight
    deconv1_weight = helper.make_tensor('deconv1.weight', TensorProto.FLOAT, resize_165_shape,  weight1.flatten().tolist())
    deconv2_weight = helper.make_tensor('deconv2.weight', TensorProto.FLOAT, resize_187_shape, weight2.flatten().tolist())
    ...
    inputs=["662", 'deconv1.weight'],  # name: ['resize节点的上一个输入','替换resize的反卷积的权重']
    outputs=["667"],  # name: resize节点的下一个节点node
    output_padding=[0, 0],
    group=1,
    kernel_shape=[2, 2],
    pads=[0, 0, 0, 0],
    ...
    # remove Resize node，通过print结果的节点index
...
if __name__=='__main__':
    ...
    onnx_path = './demo/VIA/yolox_via/yolox_s.onnx'
    model = onnx.load(onnx_path)
    node = model.graph.node
    # 搜索目标节点
    for i in range(len(node)):
